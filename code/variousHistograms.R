...
#breaks <- seq(min(samples),max(days))

hist(days, col=blue, main="number of days and sample per subject", border=blue, 
     xlab="number of days or samples", cex.main=1.6)
abline(v=mean(days), col=rgb(0,0,0.5), lty=2, lwd=1.5)
hist(samples, add=TRUE, col=red, border=red)
abline(v=mean(samples), col=rgb(0.5,0,0), lty=2, lwd=1.5)
legend("topright", legend=c("days","samples"), fill=c(blue, red), bty='n', 
       border=c(blue,red), cex=1.3)
